...
const tokenId = 222222;

const main = async () => {
    try {
        const estimatedGas = await contract.methods.transferFrom(addressFrom, addressTo, tokenId).estimateGas({
                                from: addressFrom,
                                gas: 0
                            });
        const txnCount = await web3.eth.getTransactionCount(addressFrom, "pending")
        const receipt = await contract.methods.transferFrom(addressFrom, addressTo, tokenId).send({
            nonce: txnCount,
            gasPrice: 0,
